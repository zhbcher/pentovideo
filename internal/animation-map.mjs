#!/usr/bin/env node
// animation-map.mjs — PentoVideo animation map for agents
//
...
const {
  ...
  await importPackagesOrBootstrap(["@pentovideo/producer"], {
    npmPackages: [pentovideoPackageSpec("@pentovideo/producer")],
  })
)["@pentovideo/producer"];
...
const OUT_DIR = resolve(args.out ?? ".pentovideo/anim-map");
