#!/usr/bin/env node
// contrast-report.mjs — PentoVideo contrast audit
//
...
//
// Usage:
//   node skills/pentovideo/scripts/contrast-report.mjs <composition-dir> \
...
import { pentovideoPackageSpec, importPackagesOrBootstrap } from "./package-loader.mjs";

// Use the producer's file server — it auto-injects the PentoVideo runtime
// and render-seek bridge, so raw authoring HTML works without a build step.
const packages = await importPackagesOrBootstrap(["@pentovideo/producer", "sharp"], {
  npmPackages: [pentovideoPackageSpec("@pentovideo/producer"), "sharp@0.34.5"],
});
...
const OUT_DIR = resolve(args.out ?? ".pentovideo/contrast");
