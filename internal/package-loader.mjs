...
const BOOTSTRAP_ENV = "PENTOVIDEO_SKILL_DEPS_BOOTSTRAPPED";
const BOOTSTRAP_CONFIRM_ENV = "PENTOVIDEO_SKILL_BOOTSTRAP_DEPS";
const NODE_MODULES_ENV = "PENTOVIDEO_SKILL_NODE_MODULES";
...
export function pentovideoPackageSpec(packageName) {
  const version = readBundledPentovideoVersion();
  if (!version) {
    throw new Error(
      [
        `Could not determine the bundled PentoVideo version for ${packageName}.`,
...
function resolvePackageEntry(packageName) {
  ...
  for (const base of bases) {
    ...
    try {
      return createRequire(join(normalized, "__pentovideo_skill_loader__.cjs")).resolve(
        packageName,
      );
    } catch {
...
function readBundledPentovideoVersion() {
...
function readPackageVersion(packageJsonPath) {
  try {
    ...
    if (manifest.name === "pentovideo" || manifest.name === "@pentovideo/cli") {
...
async function confirmBootstrap(packageSpecs) {
  ...
  try {
    const answer = await rl.question(
      [
        "PentoVideo helper package(s) are missing.",
...
function bootstrapWithNpmInstall(packageNames) {
  const installRoot = mkdtempSync(join(tmpdir(), "pentovideo-skill-deps-"));
