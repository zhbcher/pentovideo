...
const RUNTIME_CDN_URL =
  "https://cdn.jsdelivr.net/npm/@pentovideo/core/dist/pentovideo.runtime.iife.js";
...
function injectShaderOptionsIntoSrcdoc(
  ...
  const script = `<script data-pentovideo-player-shader-options>${lines.join("")}</script>`;
...
class PentovideoPlayer extends HTMLElement {
  ...
  constructor() {
    ...
    this.iframe.title = "PentoVideo Composition";
   ...
   * timeline tools (e.g. `@pentovideo/studio`) that need to inspect
   * the composition's DOM or read its `__player` / `__timelines`
   * runtime objects.
   *
   * **Common pitfall:** the iframe lives inside the player's Shadow DOM.
   * Passing the `<pentovideo-player>` element itself to code that expects
  ...
  private _createShaderLoader(): ShaderLoaderElements {
    ...
    root.setAttribute("data-pentovideo-ignore", "");
  ...
  /** Inject the PentoVideo runtime into the iframe if not already present. */
  ...
  private _mirrorParentMediaTime(timelineSeconds: number, options?: { force?: boolean }) {
    ...
    const requiredSamples = PentovideoPlayer.MIRROR_REQUIRED_CONSECUTIVE_DRIFT_SAMPLES;
    const threshold = PentovideoPlayer.MIRROR_DRIFT_THRESHOLD_SECONDS;
    for (const m of this._parentMedia) {
...
if (!customElements.get("pentovideo-player")) {
  customElements.define("pentovideo-player", PentovideoPlayer);
}

export { PentovideoPlayer };
