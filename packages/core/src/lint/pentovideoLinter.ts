import type { PentovideoLintFinding, PentovideoLintResult, PentovideoLinterOptions } from "./types";
...
export function lintPentovideoHtml(
  html: string,
 ...
 *
 * Call this after `lintPentovideoHtml()` and merge the findings.
 *
    ...
    try {
      ...
      if (!resp.ok) {
        findings.push({
          ...
          fixHint:
            "This script URL is not accessible. Remove it or replace with a valid URL. The PentoVideo runtime is injected automatically — do not load it manually.",
          snippet,
        });
      }
    } catch (err) {
