---
name: remotion-to-pentovideo
description: Translate an existing Remotion (React-based) video composition into a PentoVideo HTML composition. Use ONLY when the user explicitly asks to port, convert, migrate, translate, or rewrite a Remotion composition as PentoVideo (e.g. "port my Remotion project to PentoVideo"). Do NOT use when (a) authoring a NEW PentoVideo composition (even if A/B-testing a Remotion video); (b) Remotion is mentioned in passing; (c) Remotion code is shared as reference, not for translation; (d) the user wants "the same video as my Remotion one" without explicitly asking to migrate the source — treat as a fresh PentoVideo build. When in doubt, default to the `pentovideo` skill. Detects unsupported patterns (useState, useEffect side effects, async calculateMetadata, third-party React component libraries, `@remotion/lambda`) and recommends the runtime interop escape hatch instead of a lossy translation.
---

# Remotion to PentoVideo
...
- "port my Remotion project to PentoVideo"
- "convert this Remotion code to PentoVideo"
- "migrate from Remotion"
- "translate this Remotion comp"
- "rewrite this as PentoVideo HTML"

**Do NOT use this skill when:**

- (a) The user is authoring a **new** PentoVideo composition, even if they have or are A/B-testing a similar Remotion video.
...
- (d) The user asks for "the same video as my Remotion one" without explicitly asking to migrate the source — treat that as a fresh PentoVideo build.

When in doubt, default to authoring a native PentoVideo composition with the `pentovideo` skill instead.
