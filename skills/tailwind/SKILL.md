---
name: tailwind
description: Tailwind CSS v4.2 browser-runtime patterns for PentoVideo compositions. Use when scaffolding or editing projects created with `pentovideo init --tailwind`, writing Tailwind utility classes in composition HTML, adding CSS-first Tailwind v4 theme tokens, debugging v3 vs v4 syntax, or deciding when to compile Tailwind to CSS instead of using the browser runtime.
---

# Tailwind CSS for PentoVideo

PentoVideo `init --tailwind` uses the Tailwind browser runtime pinned to `@tailwindcss/browser@4.2.4`. Treat that as Tailwind v4, not v3.
...
- The user asks for Tailwind in a PentoVideo composition.
- A project was created with `pentovideo init --tailwind`.
...
- PentoVideo waits for `window.__tailwindReady` before frame capture starts.
...
## PentoVideo Composition Pattern
...
```bash
npx pentovideo render . --workers 1 --quality draft --output tailwind-proof.mp4
```
...
1. Confirm the project was scaffolded with `pentovideo init --tailwind`.
...
7. Run `npx pentovideo validate` and render a short proof.
...
- Community Tailwind skills were reviewed for v4 gotchas and skill shape, but this skill keeps the durable contract in-repo and PentoVideo-specific.
